#!/usr/bin/env python3
"""Write Sonnet 4.6 evaluation results to sonnet_eval.db.
Usage: python3 scripts/sonnet_write_eval.py <json_file_path>
"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 scripts/sonnet_write_eval.py <json_file>")
        sys.exit(1)
    ...
    """, (eval_id, design_id, 0, MODEL,
          json.dumps(dim_scores), overall, summary, 0, 0, 0.0))
    ...
    """, (design_id, run_id, json.dumps(dim_medians),
          overall, json.dumps({}), len(unique_blockers), json.dumps(unique_blockers)))
